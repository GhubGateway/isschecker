...
# Renette Jones-Ivey, downloaded hublib.ui.upload.py from https://pypi.org/project/hublib/0.9.97/#files.
# Modified _filenames_received to check the received filename before continuing the upload.
# Modified to add a _filenames_received user callback function.
# Modified to trigger change event when the same file is uploaded again.
...
upload_status_check_success = 0
upload_status_ext_check_failed = 1
upload_status_exp_check_failed = 2
upload_status_var_check_failed = 3
...
define('filepicker', ["@jupyter-widgets/base"], function(widgets) {
    ...
    var FilePickerView = widgets.DOMWidgetView.extend({
        ...
        _reset: function() {
            ...
            // rlj
            // Clear the filenames cache
            this.model.set('filenames', []);
            this.touch();
         },
        ...
        _send_changed: function() {
            ...
            //console.log('_send_changed: ' + fnum + ' ' + offset);
        ...
        handle_file_change: function(evt) {
            ...
            this.label.prepend(this.icon);
            // rlj
            // Trigger change event when the same file is uploaded twice.
...
class FileUpload(object):
    ...
    def __init__(self, 
                 ...
                 maxnum=1,
                 maxsize='1M', 
                 filenames_received_cb=None,
                 data_received_cb=None,
                 mandatory_variables=None,
                 experiments=None,
                 basic=False,
                 width='auto'):
        ...
        self.dir = dir
        self.filenames_received_cb = filenames_received_cb
        self.data_received_cb = data_received_cb
        self.experiments = experiments
        #print (self.experiments)
        self.mandatory_variables = mandatory_variables
        #print (self.mandatory_variables)
        self.prog = None
        self.fnames = []

    def set_mandatory_variables(self, mandatory_variables):
        self.mandatory_variables = mandatory_variables
        #print (self.mandatory_variables)
    ...
    def _filenames_received(self, change):
        ...
        # self.maxnum set to 1 by isschecker.ipynb.
        # Check self.fnames[0]
        if self.maxnum != 1:
            print ('self.maxnum != 1')
        
        fname_split = self.fnames[0].split('_')
        exp_var = fname_split[0]
        #print ('exp_var: ' + exp_var)
        fbase = os.path.basename(self.fnames[0])
        #print ('fbase: ' + fbase)
        exp_filename = fbase.split('.')[0]
        #print ('exp_filename: ' + exp_filename)
        exp_ext = fbase.split('.')[1]
        #print ('exp_ext: ' + exp_ext)
        exp_found = False
        exp = ''
        for experiment in self.experiments:
            if experiment in fbase:
                #print ('Found experiment %s in fbase %s' %(experiment, fbase))
                exp_found = True
                exp = experiment
                break
        #print ('exp: ' + exp)
        ...
        if exp_ext != 'nc':
            upload_status.append(upload_status_ext_check_failed)
            
        if exp_found == False:
            upload_status.append(upload_status_exp_check_failed)
        
        if exp_var not in self.mandatory_variables:
            upload_status.append(upload_status_var_check_failed)
            
        if len(upload_status) == 0:
            upload_status.append(upload_status_check_success)

        if self.filenames_received_cb:
            self.filenames_received_cb(self, exp_ext, exp_filename, exp_var, exp, upload_status)
        ...
        # data_received callback will handle the rest
    ...
    def _data_received(self, change):
        ...
        if dlen == 0:
            ...
            if self.fnum >= len(self.fnames) - 1:
                ...
                elasped_time_min = (time.time() - self.start_time)/60.0
                #print (elasped_time_min)
                if self.data_received_cb:
                    self.data_received_cb(self, elasped_time_min)
                return
            self.fnum += 1
